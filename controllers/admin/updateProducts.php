...
//POST productId gotten from the product overview page
$id = $_POST['id'];
...
$product = $PRODUCTS->showOne($id);
$nutrition = $PRODUCTS->showNutrition($id);
...
$nutritionFats = $nutrition['fats'];
$nutritionSaturated = $nutrition['saturated'];
$nutritionCarbs = $nutrition['carbohydrates'];
...
if (!empty($_POST))
{
    try
    {
        ...
        if (isset($_POST['id']) && isset($_POST['productName']) && isset($_POST['productPrice']) && isset($_POST['productDesc']) && isset($_FILES['productImg']['name']) && isset($_POST['productStock']) && isset($_POST['nutritionEnergy']) && isset($_POST['nutritionFats']) && isset($_POST['nutritionSaturated']) && isset($_POST['nutritionCarbs']) && isset($_POST['nutritionSugars']) && isset($_POST['nutritionProtein']) && isset($_POST['nutritionSalt']))
        {
            ...
            else
            {
                ...
                if(!empty($fileName))
                {
                    ...
                    else if(!file_exists($targetFilePath))
                    {
                        ...
                        if (move_uploaded_file($_FILES["productImg"]["tmp_name"][0], $targetFilePath)) {
                            $PRODUCTS = new Product($database);
                            $PRODUCTS->edit($productName, $productPrice, $productDesc, $fileName, $categoryId, $productFeature, $productStock, $productId);
                            $PRODUCTS->editNutrition($nutritionEnergy, $nutritionFats, $nutritionSaturated, $nutritionCarbs, $nutritionSugars, $nutritionProtein, $nutritionSalt, $productId);
                            return header('location: '.Request::buildUri( '/admin/producten'));
                        }
                        else {
                ...
                {
                    $PRODUCTS = new Product($database);
                    $PRODUCTS->edit($productName, $productPrice, $productDesc, $productImg, $categoryId, $productFeature, $productStock, $productId);
                    $PRODUCTS->editNutrition($nutritionEnergy, $nutritionFats, $nutritionSaturated, $nutritionCarbs, $nutritionSugars, $nutritionProtein, $nutritionSalt, $productId);
                    return header('location: '.Request::buildUri( '/admin/producten'));
                }
            }
...
require 'views/admin/updateProducts.view.php';